...
let
      ...
      makeWrapperArgs =
        let
          ...
          flags = [
            ...
            "--cmd" "set packpath^=${pkgs.vimUtils.packDir packDirArgs}"
            "--cmd" "set rtp^=${pkgs.vimUtils.packDir packDirArgs}"
            ];
        in
        [
          ...
          "--prefix" "LUA_PATH" ";" (pkgs.neovim-unwrapped.lua.pkgs.lib.genLuaPathAbsStr luaEnv)
          "--prefix" "LUA_CPATH" ";" (pkgs.neovim-unwrapped.lua.pkgs.lib.genLuaCPathAbsStr luaEnv)
        ];
  ...
  legacyWrapper = neovim: {
    ...
    /* the function you would have passed to python.withPackages */
    , withPython3 ? false,  extraPython3Packages ? (_: [])
    /* the function you would have passed to lua.withPackages */
    , extraLuaPackages ? (_: [])
    , withNodeJs ? false
    , withRuby ? false
    ...
    let
      wrapNvcodeUnstable = pkgs.callPackage ./wrapper.nix {  };
