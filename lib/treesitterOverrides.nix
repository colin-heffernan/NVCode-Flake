...
let
	generatedGrammars = let
		...
		buildGrammar = lang: pkgs.tree-sitter.buildGrammar {
			...
			location = if (lang == "tree-sitter-markdown") then "tree-sitter-markdown" else if (lang == "tree-sitter-markdown-inline") then "tree-sitter-markdown-inline" else if (lang == "tree-sitter-typescript") then "typescript" else "";
		};
	in
