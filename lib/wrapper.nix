...
let
  wrapper = {
    ...
    , manifestRc ? null
    , withPython2 ? false
    , withPython3 ? false,  python3Env ? pkgs.python3
    , withNodeJs ? false
    , rubyEnv ? null
    , vimAlias ? true
    , viAlias ? true
  ...
  let
    ...
    neovim-configs = pkgs.stdenv.mkDerivation {
    ...
    in
      ...
      ++ optionals wrapRc [ "--add-flags" "-u ${configDerivation}/init.lua --cmd 'set runtimepath^=${configDerivation}'" ];
  in
  ...
  pkgs.symlinkJoin {
      ...
      in ''
        ...
        if ! $out/bin/nvim-wrapper \
          -u ${pkgs.writeText "manifest.vim" manifestRc} \
          -i NONE -n \
          -V1rplugins.log \
